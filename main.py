    ...
    if request.method == 'POST':
        ...
        for result in flight_results:
            for flight in result:
                result[flight]['departure_city'] = CITIES[result[flight]['departure_city']][0]
                result[flight]['arrival_city'] = CITIES[result[flight]['arrival_city']][0]
                departure_minutes = result[flight]['departure_time'] * 5
                result[flight]['departure_time'] = '{:02d}'.format(departure_minutes // 60) + ":" + '{:02d}'.format(departure_minutes % 60)
                arrival_minutes = result[flight]['arrival_time'] * 5
                result[flight]['arrival_time'] = '{:02d}'.format(arrival_minutes // 60) + ":" + '{:02d}'.format(arrival_minutes % 60)
